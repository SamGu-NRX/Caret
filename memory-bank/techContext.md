...
- Chat completions use Vercel AI Gateway (`caret/completions.py`, default model `google/gemini-2.5-flash`). Set `VERCEL_API_GATEWAY_KEY` locally. Optional alias: `AI_GATEWAY_API_KEY`. The dispatch-only workflow `.github/workflows/live-smoke.yml` is the only CI job that reads `${{ secrets.VERCEL_API_GATEWAY_KEY }}`. Regular `ci.yml` stays keyless. CLI smoke: `python3 -m caret complete --prompt "Hello"`.
