...
export async function runPipeline() {
  ...
  let webhook: { url: string; status: number } | null = null;
  if (process.env.SCHEDULE_WEBHOOK_URL) {
    const response = await fetch(process.env.SCHEDULE_WEBHOOK_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ run_id: runId, holds, ics }) });
    webhook = { url: process.env.SCHEDULE_WEBHOOK_URL, status: response.status };
  }
  return {
