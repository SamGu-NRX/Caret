...
| Storage | none | Holds are returned in the response and as `.ics`; no external writes. No database. |
...
- **Nothing is sent or booked.** There is no mail or calendar connector. Holds are tentative: shown in the UI, exported as `.ics`. Fixture holds never call a calendar webhook.
