...
6. **jev-scheduler needs a context adapter.** To join the contract it must accept `{thread, history}` from a `ContextFrame` instead of `inputs/*.json`, and split `runPipeline()` into prepare (extract + plan + rank, read-only) and execute (holds/webhook), as `docs/input-pipeline.md` asks.
